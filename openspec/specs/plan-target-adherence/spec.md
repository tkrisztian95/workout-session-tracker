# plan-target-adherence Specification

## Purpose

Surface, on the stats page, how well the user is meeting their plan targets over time. Each completed session that is linked to a plan day is scored by the share of that day's core exercises that met target — reusing the same classification the per-session plan comparison shows — and those scores are trended over the selected time range alongside an average-adherence summary. The section counts only plan-linked sessions and hides itself when there are none to score.
