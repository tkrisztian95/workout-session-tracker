...
- reps vs repsPerSet: emit exactly one of these per sets-reps exercise. Preserve the original exercise's scheme style by default — if the original had repsPerSet, keep repsPerSet; if it had a uniform "reps" value, keep that. Switch only if the adjustment requires it (e.g. "make it a pyramid"). When using repsPerSet, do NOT also send "reps"; the set count is the array length.
...
- Match the set/rep/duration and intensity scheme to the original exercise and the surrounding exercises in the same day. If the original used "repsPerSet", keep the same scheme shape (same array length, same general progression).
- reps vs repsPerSet: emit exactly one of these per sets-reps replacement. Do not send both.
